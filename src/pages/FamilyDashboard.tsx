...
const FamilyDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'management' | 'progress'>('management');
  ...
  if (!currentUser?.isHeadOfHousehold || currentUser.accountType !== 'family') {
    return (
      <div className="card">
        <h1 className="text-2xl font-heading text-primary mb-4">Family Dashboard</h1>
        <p className="text-text-light font-body">
          This page is only accessible to family account managers. Please contact support if you believe this is an error.
        </p>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-text-light font-body">
                Manage members and track progress for your family ({members.length}/10)
              </p>
            </div>
          </div>

          {activeTab === 'management' && !showAddForm && members.length < 6 && (
            <button
