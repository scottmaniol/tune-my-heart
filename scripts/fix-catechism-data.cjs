/**
 * Script to fix catechism data weeks 4-52 with correct questions/answers/proof texts
 */

const fs = require('fs');
const path = require('path');

// Correct data for all 52 weeks (with empty text fields)
const correctData = [
...
console.log('Fixing catechism data...\n');

// For each week 4-52, update the question, answer, answerChild, proofTexts references, and memoryVerse reference
for (let i = 3; i < 52; i++) { // weeks 4-52 (index 3-51)
  const weekData = correctData[i];
  const weekNum = weekData.week;
  
  console.log(`Updating week ${weekNum}: ${weekData.question}`);
  
  // Create the proof texts array string
  const proofTextsStr = weekData.proofTexts.map(ref => `      {\n            reference: "${ref}",\n            text: ""\n      }`).join(',\n');
  
  // Build the regex to find this week's entry
  // We'll match from "  {" followed by week number through to the closing brace before the next week or end
  const weekRegex = new RegExp(
    `(  \\{\\s*week: ${weekNum},)[\\s\\S]*?(\\}${i < 51 ? ',' : ''}\\s*(?=\\{|\\];))`,
    'm'
  );
  ...
  const replacement = `$1
    ...
    answer: "${weekData.answer}",
    answerChild: "${weekData.answerChild}",
    proofTexts: [
${proofTextsStr}
],
    memoryVerse: {
      reference: "${weekData.memoryVerse}",
      text: ""
    }
  $2`;
...
// Also update bonus question (week 52's data)
console.log('\nUpdating bonus question...');
const bonusRegex = /export const bonusQuestion[\s\S]*?(?=export function getCatechismQuestion)/;
const bonusData = correctData[51]; // week 52
const bonusProofTexts = bonusData.proofTexts.map(ref => `    { reference: "${ref}", text: "" }`).join(',\n');

const bonusReplacement = `export const bonusQuestion: CatechismQuestion = {
  week: 0,
...
content = content.replace(bonusRegex, bonusReplacement);

// Write the updated content
fs.writeFileSync(filePath, content, 'utf8');

console.log('\n✅ Catechism data updated successfully!');
console.log('All weeks 1-52 now have the correct questions, answers, and proof text references.');
